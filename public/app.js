...
const isRevisionEl = document.getElementById('isRevision');
...
function updateCurrencyUI() {
  ...
  if (isRevisionEl.checked) {
        installmentsEl.value = 9;
        installmentsEl.disabled = true;
        if (c !== 'MYR') installmentHint.textContent = 'Revision 模式：固定分 9 期，不加手续费';
  } else {
        installmentsEl.disabled = false;
        if (installmentsEl.value === '9') installmentsEl.value = '';
  }
}
currencyEl.addEventListener('change', updateCurrencyUI);
isRevisionEl.addEventListener('change', updateCurrencyUI);
updateCurrencyUI();

function renderResult(result) {
    const c = result.currency;
    let html = '';
    html += `<div class="result-row"><span>Main Amount</span><span>${c} ${fmt(result.mainAmount)}</span></div>`;
    if (result.isRevision) {
          html += `<div class="result-row"><span>Revision（半价）</span><span>${c} ${fmt(result.mainAmount / 2)}</span></div>`;
    }
    if (result.deposit > 0) {
          html += `<div class="result-row"><span>Deposit（已扣）</span><span>- ${c} ${fmt(result.deposit)}</span></div>`;
    }
  ...
  if (c === 'MYR') {
        ...
        if (result.isRevision) {
                html += `<div class="result-row"><span>${result.installments} 期（Revision 固定），每期应付</span><span><b>MYR ${fmt(result.perInstallment)}</b></span></div>`;
        } else {
        ...
        if (!result.isRevision) {
                html += `<div class="result-row"><span>分期手续费</span><span>+${c} ${fmt(result.surcharge)}</span></div>`;
        }
        html += `<div class="total">${c} 总额：${fmt(result.total)}</div>`;
        html += `<div class="result-row"><span>${result.installments} 期${result.isRevision ? '（Revision 固定）' : ''}，每期应付</span><span><b>${c} ${fmt(result.perInstallment)}</b></span></div>`;
  }
...
function collectPayload() {
    ...
    const isRevision = isRevisionEl.checked;
    const payload = { currency, mainAmount, note, deposit, isRevision };
    if (currency === 'MYR') {
          payload.rate = document.getElementById('rate').value;
    }
    if (!isRevision && currency !== 'MYR') {
...
async function loadRecords() {
    ...
    try {
          ...
          records.forEach((r) => {
                  ...
                  if (r.currency === 'MYR') {
                            if (r.isRevision) {
                                        detail = `Revision，Rate ${r.rate}，9 期每期 ${fmt(r.perInstallment)}`;
                            } else {
                                        detail = `Rate ${r.rate}；12/24/36期：${fmt(r.breakdown['12'])} / ${fmt(r.breakdown['24'])} / ${fmt(r.breakdown['36'])}`;
                            }
                  } else {
                            detail = `${r.installments} 期${r.isRevision ? '（Revision）' : ''}，每期 ${fmt(r.perInstallment)}${r.isRevision ? '' : `（含手续费 +${fmt(r.surcharge)}）`}`;
                  }
                  if (r.deposit > 0) detail += `，Deposit -${fmt(r.deposit)}`;
                  html += `<tr>
                          <td>${time}</td>
