...
- **MYR**：需要自己填当前汇率 Rate（因为汇率每次不一样）。
  ...
  - 自动显示 **12 期 / 24 期 / 36 期** 三种每期应付金额供参考
  - **Revision + MYR**：总额取整规则相同，但目标改成个位为 **9**（整数部分个位数字是 1-8 时，向上调整到个位为 9，例如 6216.65 会变成 6219；个位是 0 时保持原样不调整），固定显示 9 期每期应付金额
...
6. 回到 Web Service 的 Environment 设置里，添加 `DATABASE_URL` 环境变量，值选择刚才建的那个 Postgres 数据库（Render 支持直接从下拉列表选数据库，会自动填连接串）。
7. 点 Deploy，等几分钟构建完成后，会给你一个 `https://xxx.onrender.com` 的网址，这就是可以分享给其他老师用的正式网址。

**关于数据保存**：现在用的是真正的 Postgres 数据库，账号和记录不会因为重新部署或服务休眠而丢失。唯一要注意的是 Render 免费版 Postgres 有 30 天有效期，到期前需要升级到付费档位（几美元/月起）才能继续保留数据，否则数据库会被删除。
